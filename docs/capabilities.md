...
The destination is [`vision.md`](vision.md). This graph records stable product
responsibilities, hard prerequisites, and falsifiable completion oracles for
the `kylet.se` proof surface. It is not a roadmap, work queue, implementation
inventory, current status report, or second product destination. Cite the
`KYLE-*` IDs below.
...
- Source code and executable contracts own interface behavior. This graph owns
  product capability decomposition and oracles only.
...
2. `Done when` is a destination oracle, not a statement about this checkout,
   an open pull request, CI, or production.
3. Cached, baked, stale, curated, and live evidence are distinct. A labelled
   fallback may preserve usefulness but cannot close a live clause.
4. Health and readiness endpoints prove only their named narrow condition. A
...
5. The owned-runner hard cut is part of `KYLE-DELIVERY`. External runner or
...
6. Current work, PR collisions, checks, deploy state, and live incidents belong
