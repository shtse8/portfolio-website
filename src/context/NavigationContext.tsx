...
export const useNavigationStore = create<NavigationState>((set, get) => ({
    ...
    if (element) {
      ...
      if (window.location.pathname !== targetPath) {
        ...
        window.history.pushState(
          { path: targetPath, section: sectionId },
          "",
          targetPath,
        );
      }
  ...
  setupIntersectionObserver: () => {
    ...
    const missingElements = sections.filter(
  ...
  setupPopStateListener: () => {
    ...
    (window as WindowWithNavigationHandlers).__navigationPopStateHandler =
      handlePopState;
  },
  ...
  cleanup: () => {
    ...
    if ((window as WindowWithNavigationHandlers).__navigationObserver) {
      const observer = (window as WindowWithNavigationHandlers)
        .__navigationObserver;
      const { sections } = get();
    ...
    if ((window as WindowWithNavigationHandlers).__navigationPopStateHandler) {
      const handler = (window as WindowWithNavigationHandlers)
        .__navigationPopStateHandler;
      if (handler) {
        window.removeEventListener("popstate", handler);
      }
      delete (window as WindowWithNavigationHandlers)
        .__navigationPopStateHandler;
    }
  },
}));
